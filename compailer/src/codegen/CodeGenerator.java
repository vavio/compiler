...
import parser.RoboLParser.Nasoka_blockContext;
import parser.RoboLParser.Nasoka_promenlivaContext;
import parser.RoboLParser.Okolina_blockContext;
...
import parser.RoboLParser.ZetonContext;
import parser.RoboLParser.ZidContext;
import semantics.Procedure;
import semantics.Variable;

public class CodeGenerator extends RoboLBaseListener{
	private AntlrLexer lexer;
	private StringBuilder builder;
	int labelStartLoopId, labelEndLoopId, labelEndCondId; 
	
	public CodeGenerator(AntlrLexer lexer) {
		super();
		this.lexer = lexer;
	...
	// ------------------------------- OKOLINA ----------------------------------------
	@Override
	public void enterOkolina_block(Okolina_blockContext ctx) {
		builder.append("envS ");
		builder.append(ctx.BROJ(0).getText());
		builder.append(" ");
		builder.append(ctx.BROJ(1).getText());
		builder.append("\n");
		super.enterOkolina_block(ctx);
	}
	
	@Override
	public void enterZid(ZidContext ctx) {
		builder.append("wallP ");
		builder.append(ctx.PRAVEC().getText());
		builder.append(" ");
		builder.append(ctx.BROJ(0).getText());
		builder.append(" ");
		builder.append(ctx.BROJ(1).getText());
		builder.append(" ");
		builder.append(ctx.BROJ(2).getText());
		builder.append("\n");
		super.enterZid(ctx);
	}
	
	@Override
	public void enterZeton(ZetonContext ctx) {
		builder.append("coinP ");
		builder.append(ctx.BROJ(0).getText());
		builder.append(" ");
		builder.append(ctx.BROJ(1).getText());
		builder.append("\n");
		super.enterZeton(ctx);
	}
	
	@Override
	public void enterNasoka_block(Nasoka_blockContext ctx) {
		builder.append("robotP ");
		builder.append(ctx.BROJ(0).getText());
		builder.append(" ");
		builder.append(ctx.BROJ(1).getText());
		builder.append("\n");
		
		//builder.append("robotD ");
		builder.append("move regD ");
		builder.append(ctx.NASOKA().getText());
		builder.append("\n\n");
		super.enterNasoka_block(ctx);
	}
	...
	@Override
	public void enterInicijaliziranje_promenliva(Inicijaliziranje_promenlivaContext ctx) {
		if (ctx.nasoka_promenliva() != null && ctx.nasoka_promenliva().getStart().getType() == JFlexToken.NASOKA.getValue()) {
			builder.append("move ");
			builder.append(getIdentifier(Integer.parseInt(ctx.ID().getText())));
			builder.append(" ");
			builder.append(ctx.nasoka_promenliva().getStart().getText());
		...
		else if (ctx.nasoka_promenliva() != null && ctx.nasoka_promenliva().getStart().getType() == JFlexToken.ID.getValue()){
			builder.append("move ");
			builder.append(getIdentifier(Integer.parseInt(ctx.ID().getText())));
			builder.append(" ");
			builder.append(getIdentifier(Integer.parseInt(ctx.nasoka_promenliva().getStart().getText())));
		...
		else if (ctx.broj_promenliva() != null && ctx.broj_promenliva().getStart().getType() == JFlexToken.BROJ.getValue()) {
			builder.append("move ");
			builder.append(getIdentifier(Integer.parseInt(ctx.ID().getText())));
			builder.append(" ");
			builder.append(ctx.broj_promenliva().getStart().getText());
			builder.append("\n");
		}
		else if (ctx.broj_promenliva() != null && ctx.broj_promenliva().getStart().getType() == JFlexToken.ID.getValue()) {
			builder.append("move ");
			builder.append(getIdentifier(Integer.parseInt(ctx.ID().getText())));
			builder.append(" ");
			builder.append(getIdentifier(Integer.parseInt(ctx.broj_promenliva().getStart().getText())));
		...
		super.enterInicijaliziranje_promenliva(ctx);
	}
	
	@Override
	public void enterAzururanje_promenliva(Azururanje_promenlivaContext ctx) {
		builder.append("push\n");
		super.enterAzururanje_promenliva(ctx);
	}
	
	@Override
	public void exitAzururanje_promenliva(Azururanje_promenlivaContext ctx) {
		builder.append("move ");
		builder.append(getIdentifier(Integer.parseInt(ctx.ID().getSymbol().getText())));
		builder.append(" regN\n");
		builder.append("pop\n");
		super.exitAzururanje_promenliva(ctx);
	}
	
	@Override
	public void enterIzraz(IzrazContext ctx) {
		builder.append("move regN ");
		if (ctx.broj_promenliva(0).getStart().getType() == JFlexToken.ID.getValue()) {
			builder.append(getIdentifier(Integer.parseInt(ctx.broj_promenliva(0).getStart().getText())));
		}
		else if (ctx.broj_promenliva(0).getStart().getType() == JFlexToken.BROJ.getValue()) {
			builder.append(ctx.broj_promenliva(0).getStart().getText());
		}
		builder.append("\n");
		
		
		if (ctx.operator().start.getType() == JFlexToken.OP_ADD.getValue()) {
			builder.append("add regN ");
		}
		else if (ctx.operator().start.getType() == JFlexToken.OP_MINUS.getValue()) {
			builder.append("sub regN ");
		}
		else if (ctx.operator().start.getType() == JFlexToken.OP_MULTIPLY.getValue()) {
			builder.append("mul regN ");
		}
		else if (ctx.operator().start.getType() == JFlexToken.OP_DIVIDE.getValue()) {
			builder.append("div regN ");
		}
		
		if (ctx.broj_promenliva(1).getStart().getType() == JFlexToken.ID.getValue()) {
			builder.append(getIdentifier(Integer.parseInt(ctx.broj_promenliva(1).getStart().getText())));
		}
		else if (ctx.broj_promenliva(1).getStart().getType() == JFlexToken.BROJ.getValue()) {
			builder.append(ctx.broj_promenliva(1).getStart().getText());
		}
		builder.append("\n");
		
		super.enterIzraz(ctx);
	}
